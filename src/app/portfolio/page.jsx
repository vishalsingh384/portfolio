            ...
            {items.map((item) => (
              <div className={`h-screen w-screen flex items-center justify-center bg-gradient-to-r ${item.color}`} key={item.id}>
                <div className='flex flex-col gap-8 text-white'>
                  ...
                  <p className='w-80 md:w-96 lg:w-[400px] xl:w-[600px]'>{item.desc}</p>
                  <Link href={item.link} className='flex justify-end'>
                    <button className='p-2 text-sm lg:py-2 lg:px-4 lg:text-md bg-white text-gray-600 font-semibold rounded'>Live</button>
                  </Link>
                </div>
              </div>
            ))}
